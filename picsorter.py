...
def nameshorten(image):
    ...
    if name_length % 2 == 0 and name_length > 50: #even
        return "{} ... {} ".format(image[:namediv_even - namediv_even / 2 ], image[- int(ceil(namediv_even / 2)):])
    elif name_length % 2 != 0 and name_length > 50: #odd
        return "{} ... {} ".format(image[:namediv_odd - namediv_odd / 2 ], image[- int(ceil(namediv_odd / 2)):])
    else:
        return image
